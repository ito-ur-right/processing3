...
void draw(){
  ...
  for(int y=0;y<winSize;y++){
    for(int x=0;x<winSize;x++){
      ...
      if(brightness(c)>88){
        ...
        img.pixels[x+y*winSize] = color(hue(c),100-noiseMap[x+y*width],brightness(c));
  ...
  for (int gridY=0; gridY<tileCount2; gridY++) {
    ...
    for (int gridX=0; gridX<tileCount2; gridX++) {
      fill(hue(picColors[noiseMap[gridX+gridY*tileCount2]*20]),100,100-noiseMap[gridX+gridY*tileCount2]);
