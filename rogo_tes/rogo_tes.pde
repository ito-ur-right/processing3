...
color shapeColor = color(0, 230, 180);
color bgColor1 = color(0,130,164);
color bgColor2 = color(200,130,0);
int fillMode = 0;
int sizeMode = 0;
...
void setup(){
  ...
  currentShape = loadShape("module_8.svg");
} 
...
void draw(){
  ...
  for (int gridY=0; gridY<tileCount; gridY++) {
    for (int gridX=0; gridX<tileCount; gridX++) {
      ...
      //shape(currentShape, 0,0, newShapeSize*(map(gridX,0,tileCount,2.0,1)),newShapeSize*(map(gridY,0,tileCount,2.0,1)));
      shape(currentShape, 0,0, newShapeSize*(map(abs(tileCount/2-gridX),0,tileCount,1.5,0)),newShapeSize*(map(abs(tileCount/2-gridY),0,tileCount,1.5,0)));
      popMatrix();
    }
  }
