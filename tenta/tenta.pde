...
void tenta(int j){
  ...
  int len = (int)random(0,height/3);
  int hl = (int)random(360);
  int hh = (int)random(360);
  ...
  for(float i= len;i>(int)random(3,10);i-=0.5){
    fill((int)map(noise(xoff),0,1,hh,hl),(int)map(i*2,len,len*2,50,0),(int)map(i*2,len,len*2,50,25));
    if(i>weight){
      size = weight;
    }else{
      size = i;
